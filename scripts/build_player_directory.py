...
def main():
    ...
    for row in rows:
        ...
        # Recent call-ups may arrive in the reference NBA-stat export before the
    ...
    static_verified=sum(p['verified'] for p in candidates)
    source_backed=sum(p['verification'] in {'official-static','source-entity'} for p in candidates)
    report={'source':'player_stats CSV + nba_api.stats.static.players','officialDirectoryAvailable':official_available,'candidateCount':len(candidates),'verifiedCount':static_verified,'sourceBackedCount':source_backed,'unverifiedCount':len(candidates)-source_backed,'staticDirectoryLagCount':source_backed-static_verified,'conflictCount':len(conflicts),'generatedAt':now}

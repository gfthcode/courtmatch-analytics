...
const manifestSchema=z.object({provider:z.literal('github-json'),status:z.enum(['live','recently-updated','stale','demo']),league:z.literal('NBA'),lastUpdated:z.string(),version:z.string(),coverage:z.string(),players:z.string(),teams:z.string(),matchups:z.string(),playerStats:z.string().optional(),playtypes:z.string().optional(),teamStats:z.string().optional(),playerCount:z.number().nonnegative(),matchupRecordCount:z.number().nonnegative(),playtypeRecordCount:z.number().nonnegative().optional(),isDemo:z.boolean()});

  ...
  return <div className="lab-home">
    <section className="lab-hero" aria-labelledby="lab-hero-title">
      <div className="lab-hero-copy">
        ...
        <h1 id="lab-hero-title">读懂每一次<br /><em>正面对位。</em></h1>
        <p className="lab-hero-subtitle">Explore how NBA players perform when directly matched against one another.</p>
        <p className="lab-hero-description">从回合样本、效率差异到防守限制，用同一套数据口径进入比赛的细部。</p>
        <div className="lab-actions">
          <Button asChild><Link to="/matchups">Explore Matchups <ArrowRight size={17} /></Link></Button>
          <Button variant="secondary" asChild><Link to="/comparison">Compare Players</Link></Button>
          <Link className="lab-text-action" to="/rankings">View Rankings <ArrowUpRight size={16} /></Link>
        </div>
      </div>
      ...
      <aside className="lab-command-panel" aria-label="球员分析搜索" style={{ background: '#fff', backgroundImage: 'none' }}>
        ...
        <SearchPlayers label="搜索球员或球队" onChoose={(player) => navigate(`/players/${player.id}?season=${season}`)} />
        <button className="lab-command-shortcut" type="button" onClick={onSearch}>
          <Search size={15} /> 打开全局球员搜索 <span>⌘ K</span>
        </button>
      </aside>
    </section>
    ...
    <section className="lab-metrics" aria-label="当前数据摘要" style={{ background: '#fff', backgroundImage: 'none' }}>
      ...
      <article><BarChart3 size={17} /><span>联盟平均效率</span><strong>{number(average)}<small>/100</small></strong><small>当前覆盖数据加权值</small></article>
    </section>

    <section className="lab-workspace" aria-label="首页分析工作台">
      <div className="lab-section-heading">
        <div><span>01 / HOT MATCHUPS</span><h2>热门直接对位</h2></div>
        <Link to="/matchups">进入分析工作台 <ArrowRight size={17} /></Link>
      </div>
    ...
    <section className="lab-impact-grid">
      <ImpactPanel label="03A / OFFENSIVE IMPACT" metric="offense" title="进攻影响力" description="至少 50 回合的球员加权汇总。" />
      <ImpactPanel label="03B / DEFENSIVE IMPACT" metric="defense" title="防守限制力" description="数值越低，代表当前数据下被攻效率越低。" />
    </section>

...
export function DataGate({children}:{children:ReactNode}){
 ...
 const {dataset,status}=query.data;const badge=status.state==='live'?'实时数据':status.state==='recently-updated'?'最近更新':status.state==='stale'?'数据质量警告':'演示数据';return <DataContext.Provider value={query.data}><div className="data-strip"><span className="mode-tag">{badge}</span><span className="data-source"><img src="https://cdn.nba.com/logos/leagues/logo-nba.svg" alt="NBA 标志" loading="lazy"/><span>{status.isDemo?'仅用于产品演示，不代表官方 NBA 追踪数据。':'NBA 官方公开数据'}</span></span><span>发布层：GitHub JSON</span><span>{dataset.seasons.length} 个赛季 / {dataset.players.length} 位球员 / {dataset.matchups.length.toLocaleString()} 条对位记录</span><span>更新 {status.lastUpdated.slice(0,10)} · v{status.version}</span>{status.message&&<span title={status.message}>备用状态</span>}</div>{children}</DataContext.Provider>;
}

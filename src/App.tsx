...
export function App(){
 ...
 useEffect(()=>{setDrawerOpen(false);window.scrollTo({top:0});const labels:Record<string,string>={'/':'NBA 直接对位分析','/matchups':'联盟对位','/comparison':'球员比较','/rankings':'排行榜','/playtypes':'Play Types','/methodology':'方法论','/sources':'数据来源','/settings':'设置'};document.title=`${labels[location.pathname.replace(/\/$/,'')||'/']??'球员数据'} | CourtMatch`;},[location.pathname]);
